...
class MoveCubeMap(dict):
    def apply_cube_turn(self, specific_cube_rotation_turn):
        print(str(specific_cube_rotation_turn))
        self.update({self.get_mapped_face(v): k for k,v in specific_cube_rotation_turn.items()})
...
class MotorMovesSolver():
    ...
    def solve(self):
        for move in self.moves_pool.get_pool():
            transformed_face = self.move_cube_map.get_mapped_face(move.face)
            print("Searching for path to {0}... -> Transformed to {1}".format(str(move.face), str(transformed_face)))
            if(transformed_face not in self.current_motor_state):
                print("No mapping to {0}. Searching for mapping....".format(str(transformed_face)))
                cube_turn_comb = self._search_cube_rotation_combination_for_needed_face_id_to_be_mapped(transformed_face)
                ...
                transformed_face = self.move_cube_map.get_mapped_face(transformed_face)
            print("Correct Mapping for {0} -> transformed to {1}. Searching path...".format(str(move.face), str(transformed_face)))
